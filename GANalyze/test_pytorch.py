...
result_dir = os.path.join("./interpolation_experiment",
...
# Setting up Assessor
# --------------------------------------------------------------------------------------------------------------
#assessor_elements = getattr(assessors, train_opts['assessor'])("./assessors/best.pth")
...
# helper function
interval = 7
interpolation_heatmaps = [np.zeros((interval,interval)), np.zeros((interval,interval)), np.zeros((interval,interval))] 
values =  np.linspace(-0.2,0.2,interval)

def make_image_heatmap(z_p, y_p):
    global interpolation_heatmaps, values

    step_size = 1

    for nonused_i in range(3):
        for i in range(interval):
            for j in range(interval):
                z = copy.deepcopy(z_p)
                y = copy.deepcopy(y_p)
                z_transformed = transformation.transform_interpolation(z, y, step_size, \
                                                       [values[i], values[j]], nonused_i)
                ...
                out_scores_current = out_scores_current.detach().cpu().numpy().mean()
                interpolation_heatmaps[nonused_i][interval-i-1][j] += out_scores_current
...
# Test settings
interpolation_exp = True if opts["which_way"] == -1 else False
truncation = opts["test_truncation"]
num_samples = 2

if interpolation_exp:
    iters = 1 # should be 1 for interpolation exp
else:
    iters = 5
...
for y in range(num_categories):
    ...
    for feed_dict in feed_dicts:
        ...
        if interpolation_exp:
            total += 1
            make_image_heatmap(feed_dict["z"], feed_dict["y"])
            continue
    ...
    if not interpolation_exp:
        ...
        if opts["one_way"] ==1:
            I.save(os.path.join(result_dir, categories[y] + "_direction" + str(opts["which_way"]) + ".jpg"))
        else:
            I.save(os.path.join(result_dir, categories[y] + ".jpg"))
        print("y: ", y)

if interpolation_exp:
    ticks = ["{:.2f}".format(i) for i in values]
    for plt_i, hm in enumerate(interpolation_heatmaps):
        hm = hm / total
        ax = sns.heatmap(hm, linewidth=0.5, yticklabels=list(reversed(ticks)), xticklabels=ticks, vmin=0.6, vmax=0.9)
        plt.savefig("interpolation_experiment_heatmaps/{}_60-90.png".format(plt_i))
        plt.close()
        ax = sns.heatmap(hm, linewidth=0.5, yticklabels=list(reversed(ticks)), xticklabels=ticks, vmin=0.6, vmax=0.85)
        plt.savefig("interpolation_experiment_heatmaps/{}_60-85.png".format(plt_i))
        plt.close()
        ax = sns.heatmap(hm, linewidth=0.5, yticklabels=list(reversed(ticks)), xticklabels=ticks)
        plt.savefig("interpolation_experiment_heatmaps/{}.png".format(plt_i))
        plt.close()
    sys.exit(0)


if opts["one_way"] ==1:
    evaluator.finish("assessor_score_plot.png", opts["checkpoint_dir"], opts["which_way"])
else:  
    evaluator.finish("assessor_score_plot.png", opts["checkpoint_dir"], -1)
