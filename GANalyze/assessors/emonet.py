...
class CreativeClassifier(torch.nn.Module):

    def __init__(self, path):
        super(CreativeClassifier, self).__init__()
...
def creative_classifier(path):
    model = CreativeClassifier(path)
    input_transform = image_transform(model)          # model.transforms
    output_transform = lambda x: torch.nn.functional.softmax(x)[:, 1] #  x.squeeze()
...
if __name__ == "__main__":
    model, _, _ = creative_classifier("best.pth")
